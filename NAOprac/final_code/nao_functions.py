...
def get_reward_nn(inputs, bin_vals):
    reward = 0
    ...
    if inputs[0] >= int((bin_vals[0])/2) - gap - gap_left and inputs[0] <= int((bin_vals[0])/2) + gap:
        if inputs[1] >= int((bin_vals[1])/2) - gap2 - gap_left and inputs[1] <= int((bin_vals[1])/2) + gap2:
            reward = 1
        else:
            reward = -1
    else:
...
def get_reward_nn_specific(curr_state, prev_state, bin_vals):
    reward = 0

    centre_p = bin_vals[0]/2
    centre_v = bin_vals[1]/2
    if abs(curr_state[0] - centre_p) < abs(prev_state[0] - centre_p):
        reward = 1
    elif abs(curr_state[1] - centre_v) < abs(prev_state[1] - centre_v):
        reward = 1
    elif abs(curr_state[0] - centre_p) > abs(prev_state[0] - centre_p):
        reward = -1
    elif abs(curr_state[1] - centre_v) > abs(prev_state[1] - centre_v):
        reward = -1
...
def update_nn(net, inputs, action, outputs, reward, future_action_value, learn_rate_update_func=0.1, learn_rate_nn=0.001, discount_factor=0.9, momentum=0.99, epochs=20):
    #print "State:", inputs
    print "Action:", action, " Reward:", reward, " Future:", future_action_value
    print "Old output:", outputs
    outputs[action] = ((1 - learn_rate_update_func) * outputs[action]) + (learn_rate_update_func * (reward + discount_factor * max(future_action_value))) # Have to pass through twice

    ds = SupervisedDataSet(3, 3)
    ds.addSample(list(inputs), outputs)

    train = BackpropTrainer(net, ds, learningrate=learn_rate_nn, momentum=momentum)
...
def update_q(q, prev_state, curr_state, action, reward, learn_rate=0.4, discount_factor=0.9, prnt=False):
    """
    Update the Q matrix
    """
    # The old state
...
def load_network(trained_on_nao=False):
    if trained_on_nao:
        net = NetworkReader.readFrom(config["trained_models_paths"]["nn_nets"] + '/trained_nn_nao.xml')
    else:
        net = NetworkReader.readFrom(config["trained_models_paths"]["nn_nets"] + '/trained_nn.xml')
    return net


def save_network(net):
    NetworkWriter.writeToFile(net, config["trained_models_paths"]["nn_nets"] + '/trained_nn.xml')  # '../nn_trained_networks/trained_nn.xml')
    print("Saving network to:", config["trained_models_paths"]["nn_nets"] + '/trained_nn.xml')


def reset_network():
    net = NetworkReader.readFrom(config["trained_models_paths"]["nn_nets"] + '/trained_nn_original.xml')
    NetworkWriter.writeToFile(net, config["trained_models_paths"]["nn_nets"] + '/trained_nn.xml')
    print("Network reset")


def load_q_matrix(p, v, a, act=2, is_nao=False, is_delay=False, is_er=False):
    extra = ""
    if is_nao:
        extra += "nao_"
    if is_delay:
    ...
    if is_er:
        extra += "er_"
    fname = config["trained_models_paths"]["q_mats"] + "/q_" + extra + str(p) + "_" + str(v) + "_" + str(a) + "_" + str(act) + ".npz"
    data_from_file = np.load(fname)
    metadata = data_from_file["metadata"]
    print "Loading: ", fname, " \nMetadata:", metadata
...
def save_q(q, p, v, a, act=2, delay=False, er=False):
    ...
    if delay:
        ...
        extra = "er_"
    np.savez(config["trained_models_paths"]["q_mats"] + "/q_nao_" + extra + str(p) + "_" + str(v) + "_" + str(a) + "_" + str(act) + ".npz", q=q)


def load_exp(p, v, a, act):
    data_from_file = np.load(config["other"]["q_experiences"] + "/nao_exp_" + str(p) + "_" + str(v) + "_" + str(a) + "_" + str(act) + ".npz")
...
def save_exp(exp, p, v, a, max_p, max_v, max_a, act):
    ...
    np.savez(config["other"]["q_experiences"] + "/nao_exp_"+str(p)+"_"+str(v)+"_"+str(a)+"_"+str(act)+".npz", exp=exp, metadata=metadata)
...
def get_action(q_mat, p, v, a, max_p, max_v, consensus=False):
    """
    Gets the action, using a consensus of surrounding cells
    """
    sumup = 0
    for i in q_mat[p][v][a]:  # Find the sum
        sumup += i
    if sumup == 0:
        ...
        action = np.argmax(q_mat[p][v][a])

    return action


class Nao:
    def __init__(self, ip, angs=10, port=9559):
        # SET UP THE NAO
    ...
    def save_state_data(self, option, np, nv, na, mp, mv, ma, desc="", nao_ip="", two_act=False, qnn=False):
        """
        Option 0 = NN
        Option 1 = Q
        """
        ...
        name = ""
        if option == 0:
            location = config["evaluation_data_paths"]["nao_nn"]
            name = config["data_file_prefix"]["nao_nn"]
        elif option == 1:
            location = config["evaluation_data_paths"]["nao_q"]
            name = config["data_file_prefix"]["nao_q"]
        cwd = os.getcwd() + "/"+location  # Directory of where data is to be saved
        dirs = os.listdir(cwd)  # List of files in that directory
        number_files = len(dirs)  # Number of files
        file_name = location + "/" + name + "_" + str(number_files) + ".json"
...
def remove_experience(p, v, a, n):
    experiences = load_exp(12, 12, 10, 2)
    rem = experiences[p][v][a][0]
    experiences[p][v][a][0] = np.delete(rem, n)
    save_exp(experiences, 12, 12, 10, 0.13, 0.5, 0.004, 2)
